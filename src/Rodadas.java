        ...
        for (int i = 0; i < decisaoJogador.length; i++) {
            try {
                ...
                if (!(opcaoJogador == 1 || opcaoJogador == 2)) {
                    ...
                    System.out.println("Jogador escolheu bet. Qual o valor da aposta? (valor da aposta atual: " + apostaDaRodada);
        ...
        do {
            for (int i = 0; i < decisaoJogador.length; i++) {
                if (decisaoJogador[i][0] == "bet") {
                    if ((int) decisaoJogador[i][1] >= apostaDaRodada) {
                        apostaDaRodada = (int) decisaoJogador[i][1];
                        apostasIguais = true;
                    } else {
                        System.out.println("Jogador " + (i + 1) + ", a aposta atual é: " + apostaDaRodada + ". Você escolhe bet(1) ou out(2)?");
                        try {
                            opcaoJogador = sc.nextInt();
                            if (!(opcaoJogador == 1 || opcaoJogador == 2)) {
                                throw new ArithmeticException("opção inválida");
